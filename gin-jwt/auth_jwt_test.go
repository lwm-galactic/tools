...
func TestLoginHandler(t *testing.T) {
	...
	authMiddleware, err := New(&GinJWTMiddleware{
		...
		LoginResponse: func(c *gin.Context, code int, token string, token1 string, t time.Time, t1 time.Time) {
	...
	r.POST("/login").
		...
		Run(handler, func(r gofight.HTTPResponse, rq gofight.HTTPRequest) {
			...
			fmt.Println(r.HeaderMap.Get("Set-Cookie"))
...
func TestClaimsDuringAuthorization(t *testing.T) {
	...
	userToken, _, _ := authMiddleware.AccessTokenGenerator(MapClaims{
